...
// 2. 接收后端传递的处理后的数据（关键：必须加|tojson|safe）
// 从window对象获取在HTML模板中注入的数据
let processedData = window.processedData || {};

// 3. 调试用：在控制台打印数据，确认是否接收到（打开F12可查看）
console.log("后端传递的数据：", processedData);
console.log("原始标签数据：", profileData.tags);

// 4. 合并数据：将后端的value值合并到前端标签中
if (processedData.tags) {
    processedData.tags.forEach(processedTag => {
...
function renderProfile() {
    ...
    profileData.categories.forEach(category => {
        ...
        if (categoryTags.length) {
            // 获取该分类对应的自定义数据
            const customData = processedData.custom_data ? processedData.custom_data[category.id] : null;
