...
def plot_destination_time_heatmap(nav_data: List[Dict[str, Any]]) -> None:
    """
    绘制目的地与时间段相关性热力图，横轴显示时间段名称和具体范围
    ...
    # 定义时间段划分函数，返回包含时间段名称和具体范围的字符串
    def get_time_period(hour):
        """将小时转换为对应的时间段，包含具体时间范围"""
        if 0 <= hour < 6:
            return "凌晨 (0-6点)"
        elif 6 <= hour < 9:
            return "清晨 (6-9点)"
    ...
    # 统计每个目的地在每个时间段的出现次数
    heatmap_data = df.groupby(['poi', 'time_period']).size().unstack(fill_value=0)
    # 按定义的顺序重新排列列
    heatmap_data = heatmap_data.reindex(columns=period_order)
    
    # 创建热力图，增加宽度以容纳更长的标签
    plt.figure(figsize=(16, 9))
    sns.heatmap(heatmap_data, cmap="YlOrRd", annot=True, fmt="g", cbar_kws={'label': '出现次数'},annot_kws={"size": 4})
    plt.title('目的地与时间段相关性热力图', fontsize=15)
    plt.xlabel('时间段', fontsize=12)
    plt.ylabel('目的地', fontsize=12)
    
    # 旋转x轴标签并调整对齐方式，避免文字重叠
...
def draw_sunburst_chart(data_list):
    # 将输入数据列表转换为 DataFrame
    input_df = pd.DataFrame(data_list)

    # 合并输入数据和文件数据
    merged_df = input_df.merge(df, left_on='end_typeCode', right_on='NEW_TYPE', how='left')

    # 统计大类的数量
    big_category_count = merged_df['大类'].value_counts().reset_index(name='数量')

    # 统计中类的数量
    mid_category_count = merged_df.groupby(['大类', '中类']).size().reset_index(name='数量')

    # 统计小类的数量
    sub_category_count = merged_df.groupby(['大类', '中类', '小类']).size().reset_index(name='数量')

    # 构建 Sunburst 数据结构
    big_category_data = []
    for _, row in big_category_count.iterrows():
        big_category = row['大类']
        mid_categories = mid_category_count[mid_category_count['大类'] == big_category]
        mid_category_nodes = []
        for _, mid_row in mid_categories.iterrows():
            mid_category = mid_row['中类']
            sub_categories = sub_category_count[
                (sub_category_count['大类'] == big_category) & (sub_category_count['中类'] == mid_category)]
            sub_category_nodes = [{"name": sub_row['小类'], "value": sub_row['数量']} for _, sub_row in
                                  sub_categories.iterrows()]
            mid_category_nodes.append({"name": mid_category, "value": mid_row['数量'], "children": sub_category_nodes})
        big_category_data.append({"name": big_category, "value": row['数量'], "children": mid_category_nodes})

    # 创建 Sunburst 图表
    (
        Sunburst(init_opts=opts.InitOpts(width="1200px", height="1000px"))
        .add(
            series_name="类型分布",
            data_pair=big_category_data,
            radius=[0, "95%"],
        ...
        .set_global_opts(
            title_opts=opts.TitleOpts(title="多层级类型分布扇形图"),
            toolbox_opts=opts.ToolboxOpts(is_show=True)
        )
        .render("sunburst_chart.html")
    )
